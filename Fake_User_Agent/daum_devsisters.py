...
url = 'http://finance.daum.net/api/quotes/A194480?summary=false&changeStatistics=true'
headers = {
...
if response.status_code == 200:
    ...
    print('\nDevSisters Company Finance Crawling Starts\n', end='\r')
    time.sleep(0.5)

    for key, value in relevant_data.items():
        translated_key = translations.get(key, key)

        if key == 'tradePrice' and value >= 38000:
            print('\nDevSisters over 38000\n')

            url = 'http://finance.daum.net/quotes/A194480#home'
        ...
        if translated_key == '주당 배당금':
            print('현재 가격은\n*************************')
            print(relevant_data['tradePrice'])
            print('*************************')
            break
